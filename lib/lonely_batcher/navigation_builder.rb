module LonelyBatcher
  class NavigationBuilder < RSLT::Stylesheet
    def rules
      ...
      render('taxonomies > taxonomy') { builder.div { child_content } }
      render('taxonomy > taxonomy_name') { builder.h4(style: 'display:none;') { child_content } }
      render('taxonomy > node') { builder.ul(class: 'navigation') { child_content } }
      render('node > node') { builder.ul { child_content } }
      render('node > node_name') { builder.li {
